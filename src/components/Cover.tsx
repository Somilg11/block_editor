    ...
    <div className={`relative w-full h-[35vh] bg-neutral-300 ${!url?'hidden':''}`}>
        {!!url && (
            ...
            <div className='absolute w-[20%] h-[20%] right-0 bottom-0 group flex justify-center items-center bg-yellow-400'>
                <UploadButton className='w-fit opacity-0 group-hover:opacity-100 transition-opacity ut-allowed-content:hidden ut-button:bg-neutral-200 ut-button:hover:bg-neutral-300 ut-button:text-neutral-800 ut-button:transition-colors' endpoint='imageUploader'
                onClientUploadComplete={(res) => {
                    setUrl && setUrl(res[0].url);
                } }
                />
            </div>
            </>
